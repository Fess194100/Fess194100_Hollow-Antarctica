    ...
    public class BreezeSystem : MonoBehaviour, BreezeDamageable
    {
        ...
        private void Update()
        {
            ...
            if (AIConfidence == BreezeEnums.AIConfidence.Brave)
            {
                ...
                Combating = anim.GetBool("Combating");
        ...
        private void UpdateMovement()
        {
            ...
            /*# region Smooth transition between movement animations
            ...
            {
                case BreezeEnums.MovementType.Running:
                    if (!UseRootMotion)
                    {
#if !Breeze_AI_Pathfinder_Enabled
                        nav.speed = RunSpeed * speedMultiplier;
#else
            nav.maxSpeed = RunSpeed;
#endif
                    }
                ...
                case BreezeEnums.MovementType.Walking:
                    if (!UseRootMotion)
                    {
#if !Breeze_AI_Pathfinder_Enabled
                        nav.speed = WalkSpeed * speedMultiplier;
#else
            nav.maxSpeed = WalkSpeed;
#endif
                    }
                ...
                case BreezeEnums.MovementType.Backaway:
                    if (GetTargetDistance() > EnemyTooCloseDistance)
                    {
                        CurrentMovement = BreezeEnums.MovementType.None;
                    }
                    ...
                    {
#if !Breeze_AI_Pathfinder_Enabled
                        nav.speed = WalkBackwardsSpeed * speedMultiplier;
#else
            nav.maxSpeed = WalkBackwardsSpeed;
#endif
                    }
                ...
                case BreezeEnums.MovementType.None:
                default:
                    SetNavMovement();
                    ResetNav();
                    break;
        ...
        public void SetSpeedMultiplier(float valueMultiplier = 1f)
        {
            speedMultiplier = valueMultiplier;
        }
